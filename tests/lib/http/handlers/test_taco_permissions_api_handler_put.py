...
def handler(bot, settings, permissions_db):
    handler = TacoPermissionsApiHandler(bot, settings=settings, permissions_db=permissions_db)
...
@pytest.mark.asyncio
async def test_put_success(handler):
    ...
    with patch("bot.lib.http.handlers.api.v1.TacoPermissionsApiHandler.SimpleStatusResponse", lambda d: d):
        response = await handler.put(request, uri_variables)
...
@pytest.mark.asyncio
async def test_put_invalid_token(handler):
    handler.validate_auth_token.return_value = False
    handler._create_error_response = MagicMock(
        return_value=HttpResponse(401, headers=HttpHeaders(), body=b'{"error": "Invalid authentication token"}')
    )
    request = MagicMock(spec=HttpRequest)
    uri_variables = {"guildId": "123", "userId": "456", "permission": "admin"}
...
@pytest.mark.asyncio
async def test_put_not_found(handler):
    handler.validate_auth_token.return_value = True
...
@pytest.mark.asyncio
async def test_put_exception(handler):
    handler.validate_auth_token.return_value = True
