...
def resolver(mock_get_settings, mock_freegame_db, mock_discord_helper):
    ...
    return GuildResolver(
        ...
        freegame_db=mock_freegame_db,
        discord_helper=mock_discord_helper,
    )
...
class TestGetGuildConfig:
    ...
    def test_returns_none_when_disabled(self, resolver, mock_get_settings):
        ...
        result = resolver._get_guild_config(
            guild_id=111,
            game_id=222,
            settings_section="free_games",
        )

        assert result is None
        mock_get_settings.assert_called_once_with(111, "free_games")

    def test_returns_none_when_game_already_tracked(
        self,
        resolver,
        mock_get_settings,
        mock_freegame_db,
    ...
    def test_returns_none_when_no_channel_ids(
        self,
        resolver,
        mock_get_settings,
    ):
    ...
    def test_returns_none_when_channel_ids_missing(
        self,
        resolver,
        mock_get_settings,
    ):
    ...
    def test_returns_config_when_eligible(
        self,
        resolver,
        mock_get_settings,
    ):
    ...
    def test_returns_config_with_empty_notify_roles(
        self,
        resolver,
        mock_get_settings,
    ):
...
class TestResolveChannels:
    ...
    @pytest.mark.asyncio
    async def test_resolves_single_channel(
        self,
        resolver,
        mock_discord_helper,
        mock_text_channel,
    ...
    @pytest.mark.asyncio
    async def test_resolves_multiple_channels(
        self,
        resolver,
        mock_discord_helper,
        mock_text_channel,
    ...
    @pytest.mark.asyncio
    async def test_filters_out_none_channels(
        self,
        resolver,
        mock_discord_helper,
        mock_text_channel,
...
class TestResolveEligibleGuilds:
    ...
    @pytest.mark.asyncio
    async def test_filters_out_disabled_guilds(
        self,
        resolver,
        mock_guild,
        mock_get_settings,
        ...
        def mock_settings(guild_id, section):
            return {
                111: {"enabled": True, "channel_ids": [123]},
                222: {"enabled": False, "channel_ids": [456]},
            }.get(guild_id, {})
    ...
    async def test_handles_guild_with_some_invalid_channels(
        ...
        mock_get_settings.return_value = {
            "enabled": True,
            "channel_ids": [123, 456, 789],  # Only 123 is valid
        }
    ...
    async def test_preserves_guild_order(
        ...
        guilds = [
            mock_guild(333, "Guild C"),
            mock_guild(111, "Guild A"),
            mock_guild(222, "Guild B"),
        ]
