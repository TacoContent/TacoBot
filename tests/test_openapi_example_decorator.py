...
def test_example_schema_with_different_model():
    ...
    @example(name="role_example", schema=MockRole, placement='response', status_code=200)
    def handler():
        pass
...
def test_example_schema_placement():
    ...
    @example(name="default_role", value={"id": "1", "name": "User"}, placement='schema')
    def handler():
        pass
...
def test_example_with_single_method():
    ...
    @example(name="get_response", value=[], placement='response', status_code=200, methods=HTTPMethod.GET)
    def handler():
        pass
...
def test_multiple_examples_on_same_handler():
    ...
    @example(name="empty", value=[], summary="No results", placement='response', status_code=200)
    @example(
        name="populated",
        value=[{"id": "1"}, {"id": "2"}],
...
def test_example_without_optional_fields():
    ...
    @example(name="minimal", value={"status": "ok"}, placement='response', status_code=200)
    def handler():
        pass
...
def test_example_no_source_raises_error():
    ...
    with pytest.raises(ValueError, match="One of 'value', 'externalValue', or 'schema' must be provided"):
        ...
        @example(name="invalid", placement='response', status_code=200)
        def handler():
            pass
...
def test_example_response_without_status_code_raises_error():
    ...
    with pytest.raises(ValueError, match="status_code is required when placement='response'"):
        ...
        @example(name="invalid", value={"test": "data"}, placement='response')
        def handler():
            pass
...
def test_example_parameter_without_parameter_name_raises_error():
    ...
    with pytest.raises(ValueError, match="parameter_name is required when placement='parameter'"):
        ...
        @example(name="invalid", value=100, placement='parameter')
        def handler():
            pass
...
def test_example_all_placements():
    ...
    @example(name="param_ex", value=10, placement='parameter', parameter_name='page')
    @example(name="req_ex", value={"body": "data"}, placement='requestBody')
    @example(name="resp_ex", value={"result": "success"}, placement='response', status_code=200)
    @example(name="schema_ex", value={"default": "value"}, placement='schema')
    def handler():
        pass
    ...
    complex_data = {
        "user": {
            ...
            "profile": {
                "name": "John Doe",
                "roles": ["admin", "user"],
                "metadata": {"created": "2024-01-01", "settings": {"theme": "dark"}},
            },
        },
        "stats": [1, 2, 3, 4, 5],
    }
