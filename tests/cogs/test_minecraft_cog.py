...
class TestMinecraftCogWhitelistCallback:
    """Tests for whitelist yes/no callback function."""
...
class TestMinecraftCogStopCommandErrors:
    ...
    @patch('bot.cogs.minecraft.requests.post')
    @patch('bot.cogs.minecraft.requests.get')
    async def test_stop_server_http_error(self, mock_get, mock_post, cog, context, entity_helper):
        """Test stop server when HTTP error occurs."""
        cog.get_cog_settings.return_value = {"enabled": True, "output_channel": 22222}
        entity_helper.get_or_fetch_channel.return_value = context.channel
        ...
        mock_stop_response = MagicMock()
        mock_stop_response.status_code = 500
        mock_post.return_value = mock_stop_response
    ...
    @patch('bot.cogs.minecraft.requests.post')
    @patch('bot.cogs.minecraft.requests.get')
    async def test_stop_server_api_error(self, mock_get, mock_post, cog, context, entity_helper):
        """Test stop server when API returns failure status."""
        cog.get_cog_settings.return_value = {"enabled": True, "output_channel": 22222}
        entity_helper.get_or_fetch_channel.return_value = context.channel
        ...
        mock_stop_response = MagicMock()
        mock_stop_response.status_code = 200
        mock_stop_response.json.return_value = {"status": "failure", "message": "Server stop failed"}
        mock_post.return_value = mock_stop_response
                        ...
                        with patch("bot.cogs.minecraft.EntityHelper", return_value=entity_helper):
                            with patch("bot.cogs.minecraft.ContextHelper"):
                                with patch("bot.cogs.minecraft.PromptHelper"):
                                    ...
                                    from bot.cogs.minecraft import setup

                                    await setup(bot)
                                    bot.add_cog.assert_called_once()
                                    added_cog = bot.add_cog.call_args[0][0]
                                    assert isinstance(added_cog, MinecraftCog)
