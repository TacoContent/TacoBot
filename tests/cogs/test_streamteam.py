...
@pytest.fixture
def cog(bot, messaging, entity_helper, message_helper, tracking_db, twitch_db, settings):
    """Create StreamTeamCog instance with all required dependencies."""
    # Configure settings to return streamteam configuration
    settings.get_settings = MagicMock(
        return_value={"emoji": ["star"], "name": "Team", "message_ids": ["123"], "log_channel": "456"}
    )
    return StreamTeamCog(
        bot=bot,
        messaging=messaging,
