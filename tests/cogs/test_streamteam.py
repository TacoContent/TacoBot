...
def bot():
    ...
    bot.settings.get_settings = MagicMock(return_value={"emoji": ["star"], "name": "Team", "message_ids": ["123"], "log_channel": "456"})
    bot.settings.get_string = MagicMock(return_value="msg")
    return bot

@pytest.fixture
def cog(bot):
    with patch("bot.cogs.streamteam.Messaging"), \
...
async def test_on_raw_reaction_remove_success(cog):
    ...
    settings_dict = {"enabled": True, "some_setting": "value", "emoji": ["star"], "name": "team", "message_ids": ["123"], "log_channel": "456"}
