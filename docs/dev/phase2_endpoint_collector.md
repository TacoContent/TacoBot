...
- **`extract_openapi_block(doc: Optional[str]) -> Dict[str, Any]`**
...
- **`resolve_path_literal(node: ast.AST) -> Optional[str]`**
...
- **`collect_endpoints(handlers_root, *, strict, ignore_file_globs) -> Tuple[List[Endpoint], List[...]]`**
...
- `test_swagger_sync_component_only_update.py::test_component_only_update_triggers_write`
  - Issue: Test copies script to temp dir without package structure
  - Not related to endpoint_collector changes

- `test_swagger_sync_markers.py::test_custom_markers_parse`
...
- `test_swagger_sync_no_color.py::test_no_color_flag_behavior`
