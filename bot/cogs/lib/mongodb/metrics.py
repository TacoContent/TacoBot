        ...
        try:
                ...
                return self.connection.game_keys.aggregate(
                    [
                        ...
                        {
                            "$group": {
                            "_id": {"guild_id": "$guild_id", "user_id": "$redeemed_by"},
                            "total": {"$sum": 1}
                            }
                        },
                        {
            ...
            self.log(
                guildId=0,
                level=loglevel.LogLevel.ERROR,
                method=f"{self._module}.{self._class}.{_method}", message=f"{ex}", stackTrace=traceback.format_exc(),
            )
            return None
        ...
        try:
                ...
                return self.connection.game_keys.aggregate(
                    [
                        ...
                        {
                            "$group": {
                            "_id": {"guild_id": "$guild_id", "user_id": "$user_owner"},
                            "total": {"$sum": 1}
                            }
                        },
                        {
