...
#             async def ask_to_exempt():
#                 await self.discord_helper.ask_yes_no(
#                     ctx=ctx,
#                     targetChannel=ctx.channel,
#                     title="Pay to Post",
#                     question="Do you want to exempt a role from paying to post in this channel?",
#                     result_callback=yes_no_result,)

#             async def yes_no_result(result):
#                 async def role_result(role):
#                     if role:
#                         # if we have settings already, remove it to add the new one
...
#                         # need to get a list of roles that are exempt
#                         exempt_roles = []
#                         for c in post_settings["channels"]:
#                             if c["id"] == str(channel.id):
#                                 exempt_roles = c["exempt"]
#                                 break
#                         exempt_roles.append(str(role.id))
...
#                     await self.discord_helper.ask_role_list(
#                         ctx=ctx,
#                         title="Pay to Post",
#                         message="Please select a role to exempt from paying to post.",
#                         select_callback=role_result,)
#                     await ask_to_exempt()
