...
class TechThursdays(commands.Cog):
    ...
    @commands.guild_only()
    async def techthurs(self, ctx: ComponentContext):
        ...
        try:
            ...
            out_message = self.settings.get_string(guild_id, "techthurs_out_message", taco_count=amount, taco_word=taco_word)
            techthurs_message = await self.discord_helper.sendEmbed(channel=out_channel,
    ...
    async def _on_raw_reaction_add_give(self, payload):
        ...
        if not already_tracked:
            ...
            self.log.debug(guild_id, _method, f"Message {payload.message_id} has already been tracked for techthurs. Skipping.")
    ...
    def _import_techthurs(self, message: discord.Message):
        ...
        self.log.debug(guild_id, "techthurs._import_techthurs", f"Importing techthurs message {message_id} from channel {channel_id} in guild {guild_id} for user {message_author.id} with text {text} and image {image_url}")
        self.db.save_techthurs (
            guildId=guild_id,
            message=text or "",
            image=image_url,
    ...
    async def give_user_techthurs_tacos(self, guild_id, user_id, channel_id, message_id):
        try:
            ...
            if not channel:
                self.log.warn(guild_id, "techthurs.give_user_techthurs_tacos", f"No output channel found for guild {guild_id}")
                return
            message = None
            # get message
