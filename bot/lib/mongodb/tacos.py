...
class TacosDatabase(Database):
    ...
    def get_total_gifted_tacos(self, guild_id: int, channel: str, timespan_seconds: int = 86400) -> int:
        _method = inspect.stack()[0][3]
        try:
    ...
    def get_total_gifted_tacos_to_user(self, guild_id: int, channel: str, user: str, timespan_seconds: int = 86400) -> int:
        _method = inspect.stack()[0][3]
        try:
            if self.connection is None:
                self.open()
